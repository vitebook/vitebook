...
export const PLUGIN_NAME = '@vitebook/plugin-markdown-preact' as const;
  ...
  return {
    ...
    async configureApp(_app) {
      ...
      if (!prefreshPlugin) {
        throw logger.createError(
          ...
          )} requires the ${kleur.bold('@vitebook/plugin-preact')} plugin.`
        );
      }
    ...
    async resolvePage({ filePath, relativeFilePath }) {
      if (filter(filePath)) {
        files.add(filePath);
        return {
          id: '@vitebook/plugin-preact/client/PreactAdapter.vue',
          type: 'preact:md',
          context: {
