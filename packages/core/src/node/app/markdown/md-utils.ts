...
import { htmlEscape, isFunction, MarkdownHeader } from '../../../shared';
...
export function dedupeHoistedTags(tags: string[] = []): string[] {
  ...
  tags.forEach((tag) => {
    if (OPENING_SCRIPT_MODULE_TAG_RE.test(tag)) {
      ...
      merge(
        'svelte:head',
        tag,
        OPENING_SVELTE_HEAD_TAG_RE,
        CLOSING_SVELTE_HEAD_TAG_RE,
      );
    } else {
...
const SVELTE_TEMPLATE_TAG_RE =
  /(\{#(if|each|await|key).*\})|(\{:(else|then|catch).*\})|(\{\/(if|each|key|await)\})|(\{@(html|debug|const).*\})/gim;

export function commentOutSvelteTemplateTags(source: string) {
  return source.replace(SVELTE_TEMPLATE_TAG_RE, (match) => {
    return `<!--&%& ${match} &%&-->`;
  });
}

const SVELTE_TEMPLATE_TAG_COMMENT_RE = /(<!--&%&\s)|(\s&%&-->)/gim;

export function uncommentSvelteTemplateTags(source: string) {
  return source.replace(SVELTE_TEMPLATE_TAG_COMMENT_RE, '');
}
