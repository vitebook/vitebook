...
export function createStaticBuildAdapter(): BuildAdapterFactory {
  return (app, bundles, build, $) => {
    ...
    return {
      ...
      async write() {
        ...
        const pageCount = $.color.underline(build.renders.size);
        buildingSpinner.start(
          $.color.bold(`Building ${pageCount} HTML pages...`),
        );
        ...
        const redirectsScriptTag = `<script id="__VBK_REDIRECTS_MAP__">window.__VBK_REDIRECTS_MAP__ = ${serializedRedirectsTable};</script>`;
        ...
        buildingSpinner.stopAndPersist({
          text: $.color.bold(`Built ${pageCount} HTML pages`),
          symbol: $.icons.success,
        });
        ...
        await Promise.all(
          files.map(async ({ filename, content }) => {
            const filePath = app.dirs.out.resolve(filename);
